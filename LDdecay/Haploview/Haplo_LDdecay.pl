...
my $usage = <<U;
    \nUsage:
        ...
        # Draw pic
        -bin: calc mean r2 in interval of bin [0.3]kb

    e.g: perl Haplo_LDdecay.pl -vcf bw.vcf -info group.info -c 500 -dis 800 -skip -bin 0.3 -block
     or: perl Haplo_LDdecay.pl -vcf bw.vcf -info group.info -c 500 -dis 800 -geno 0.1 -maf 0.01 -hw 0.001 -miss 0.3 -me 1 -block -bin 0.3
U

print $usage and exit if (@ARGV == 0);

my($info,$vcf,$cut,$dis,$geno,$maf,$hw,$miss,$skip,$mendel,$block,$bin);
...
foreach my $k (keys %gp_info) {
    mkdir "$dir_draw/$k";
    chdir "$dir_draw/$k";
    `$DATA $dir_deal/$k/sort_hap.ld $bin $k\.result`;
}