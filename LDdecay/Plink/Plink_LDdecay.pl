...
my $usage = <<U;
    ...
    e.g: perl Plink.pl -info group.info -vcf bw.vcf -c 500 -kb 800 -r2 0 -n 9999 -maf 0.01 -bin 0.3 
U

print $usage and exit if (@ARGV == 0);

my($info,$vcf,$cut,$kb,$n,$r2,$geno,$maf,$hw,$mind,$mendel,$bin);
...
foreach my $k (keys %gp_info) {
    mkdir "$dir_draw/$k";
    chdir "$dir_draw/$k";
    `$DATA $dir_deal/$k/sort_plink.ld $bin $k\.result`;
}
